...
let counts = 12;
let total = 0;
let selectedSeats = []; // Store selected seats to easily cancel them later

// Helper function to set inner text
...
for (const btn of allBtn) {
  btn.addEventListener("click", function (e) {
    ...
    if (btn.classList.contains("selected")) {
      // If selected, remove the "selected" class and cancel seat selection
      btn.classList.remove("selected");

      // Remove the seat from the confirmation list
      const seatDivs = selectSeat.getElementsByTagName("div");
      for (const div of seatDivs) {
        if (div.innerText.includes(seatName)) {
          selectSeat.removeChild(div);
        }
      }
      ...
      setInnerText("cast", total);
      setInnerText("grand-cast", total);

      // Enable all buttons if the total count is less than 4
      for (const btn of allBtn) {
        btn.disabled = false;  // Enable buttons if count is less than 4
      }
    } else {
      // If not selected and less than 4 seats are selected, add the "selected" class and select the seat
      if (count < 4) {
        btn.classList.add("selected");

        // Add the seat to the confirmation list
        ...
        // Append to confirm-seat section
